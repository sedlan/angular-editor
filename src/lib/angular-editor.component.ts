...
export class AngularEditorComponent implements OnInit, ControlValueAccessor, AfterContentInit {
    ...
    private afterInit: (element: ElementRef) => void;
     ...
     * focus event
     */
    onTextAreaFocus(target: HTMLElement): void {
        // the following 'if' is added to fix innerHTML produced by contentEditable
        // when the first line entered is not enclosed in <div></div>
     ...
     * @param target html element that was changed
     */
    onContentChange(target: HTMLElement): void {
        let html = target.innerHTML;
        // the following 'if' is added to fix innerHTML produced by contentEditable
        // when the first line entered is not enclosed in <div></div>
        if (target.innerHTML === '' || target.innerHTML === '<br>') {
            this.editorService.insertHtml('<div><br></div>');
        }
